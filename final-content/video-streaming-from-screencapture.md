...
tags:
  ...
  - performance
premium: true
summary: "Design a video streaming application like Netflix and YouTube."
free_preview: false
status: draft
---
...
![Netflix recommendations page](video-streaming-pdf-assets/figures/netflix-recommendations-page.png)


![Netflix watch page](video-streaming-pdf-assets/figures/netflix-watch-page.png)
...
We scope the exercise by clarifying which product surfaces are in play (discovery, playback, billboard autoplay), which devices to target, and which streaming formats and media APIs belong in the interview.
...
Background
Since media playback on the web is a fairly specialized domain that most people would not have much experience in, we've provided a summary of the important technicalities you need to be aware of regarding video playback in the context of a system design interview. In fact, most of the content covered here is more than what is expected of candidates, but it doesn't hurt to know more.
...
The most basic way of playing videos on websites is to use a <video> HTML tag on a page with the src attribute pointing to an mp4 or webm file, just like the <img> tag. However, this most basic way of playing videos on a webpage doesn't offer the best user experience as it does not support adaptive bitrates.

Sophisticated video players on Netflix and YouTube leverage the following key components:

Player interface: This includes the user interface of the video player, which provides controls like play, pause, and volume adjustment. Browsers offer a basic playback controls UI but often you'd want to have more control over the styling and appearance.
...
Like WebP, WebM was also developed by Google as a performant file format for media on the web. WebM is more suited for web-based applications with an emphasis on open source and efficient streaming, while MP4 is a versatile format with broad device and platform support, making it a popular choice for a wide range of video applications.

## Architecture / high-level design

The architecture follows from the product: the discover page is content-heavy and SEO-relevant, while the playback page wraps a stateful media player. Both share a common shell but diverge on rendering model and data flow.

### Rendering approach

Video streaming applications have the following characteristics:

Video titles are searchable via search engines. YouTube videos are mostly public while Netflix has a title page containing just the video details and some art/thumbnails (Netflix title page example).
...
Fast initial loading speed and video startup speed are desired.
...
YouTube discovery page SSR

Netflix SSRs the entire video title page.

![Netflix title page SSR](video-streaming-pdf-assets/figures/netflix-title-page-ssr.png)
...
Default to CSR for the watch page and SSR only for discovery surfaces
Streaming playback is driven by JavaScript and the Media Source API, so SSR cannot ship a playable frame anyway. Reserve SSR budget for the discovery feed and public title pages where SEO and above-the-fold content actually benefit.

Netflix discovery page SSR

YouTube SSRs a static preview of the video (poster image) while Netflix does not SSR anything visible.

![YouTube watch page SSR](video-streaming-pdf-assets/figures/youtube-watch-page-ssr.png)


However, Netflix's initial HTML contains data needed by the React application to boot up the video player on the page. If this data is not present in the initial HTML, the page needs to make a request to fetch the data, which requires an extra round trip and will be slower.

Netflix watch page SSR
...
![Video streaming client architecture](video-streaming-pdf-assets/figures/video-streaming-client-architecture.png)


Server: Provides HTTP APIs to fetch video recommendations and video object metadata.
Video CDN server: CDN server to fetch the video contents itself. Allows fetching individual segments of the video.
Client store: Stores data needed across the whole application. Most of the data in the store will be server-originated data needed by the video recommendations page. Persists data across navigation so that revisiting the recommendations page will not require fetching the recommendations list again.
Discover page: The page users use to browse the recommended videos.
...
Watch page: The page where users watch a full video.
...
| --- | --- | --- | --- |
| Recommendations | Server | Discover page | lists (list of VideoLists, pagination (pagination metadata) |
| --- | --- | --- | --- |
| VideoList | Server | Discover page | videos (list of VideoMetadatas, pagination (pagination metadata) |
| --- | --- | --- | --- |
| VideoMetadata | Server | Discover page | id, title, boxart_url, etc. |
The video recommendations should be stored in the client store, which is preserved across page navigations. This acts as a cache to allow presenting the video recommendations immediately (without a network request) whenever the user goes back to the "Discover page" to browse more recommendations.
...
Client store entity relationships for discover and watch surfaces
Video players contain standalone data models and APIs, which will be covered in a dedicated section below.

## Interface definition (API)

The client talks to two main API surfaces: recommendations for the discover page, and manifest and segment endpoints for streaming playback.
...
Is used to SSR the initial HTML (refer to image in rendering approach section).
Rendered as JSON data within <script> tags and is injected into the client store (window.netflix.reactContext).
Data for subsequent pages is fetched from an HTTP API and added into the client store, which is then added to the page's DOM.

Media streaming and subtitles API
The APIs for streaming the video data, audio data, and subtitles for a video depend on the selected streaming protocol (DASH, HLS), which is covered in more detail below.

Video player API
Video players are covered in a dedicated section below.

Video player data model, architecture, and API
As a video player involves multiple properties (state), lots of actions resulting in state changes, and many components that rely on the central video player state, a unidirectional reducer + actions pattern is appropriate. This can be implemented using useReducer in React or React + Redux, where Redux provides more structure around the actions and reducers along with additional devtools for an enhanced developer experience.

Video player UI example and its parts

![Video player architecture](video-streaming-pdf-assets/figures/video-player-architecture.png)
...
Background events: loadstart, loadeddata, ended, error, stalled, waiting, etc. See the full list of events available on the <video> element.
A Flux-like unidirectional flow model works well here. In a reducer pattern, newState = reducer(action, state). Actions are operations that mutate state. A list of actions, the known operations that can modify state, is defined. The only way to change the state is to dispatch an action; there is no way to update the state directly. This helps to centralize state mutation logic within the reducer.
...
Treat the video element as a controlled component, not the source of truth
External inputs like media keys on keyboards, OS-level controls, and Picture-in-Picture can mutate HTMLVideoElement state directly. If the UI reads from the DOM in some paths and from component state in others, the two drift and the player appears to glitch. Pick the component state as the single source of truth and reconcile DOM events back into it.
...
The heart of the design is the video player itself, so this section starts with native browser playback before moving into streaming protocols, adaptive quality, subtitles, performance, and player UX:

Understanding native HTML <video> elements: This section explains progressive downloading, <video> attributes, methods, events, and the limits of basic browser playback.
Video streaming: This section covers Media Source Extensions, adaptive bitrate streaming, media capabilities, streaming protocols, and manifest files.
Subtitles / Closed Captions: This section discusses subtitle file delivery, embedded captions, separate caption APIs, accessibility features, and multilingual support.
Performance: This section explains startup latency, buffering, lifecycle separation, stream selection, image optimization, bandwidth efficiency, and memory usage.
User experience: This section covers playback controls, customization, seek behavior, thumbnails, and details that shape the watching experience.
Accessibility (a11y): This section discusses captions, visual assistance, screen-reader support, keyboard controls, and external control surfaces.
Internationalization (i18n): This section covers multilingual player UI, audio tracks, subtitles, and regional formatting concerns.
Bonus: This section collects extra production-grade details such as seek-bar thumbnails and homepage autoplay previews.
...
The simplest way to render a video is similar to images, where the src attribute points to a video file.
...
Seeking can be achieved by using an HTTP Range request to download the appropriate segment. An HTTP Range request asks the server to send only a portion of an HTTP message back to the client, which is useful for media players because they want to support random access of a file.

Video playback on Netflix and YouTube uses streaming as opposed to progressive downloading. It is achieved through the Media Source API in combination with adaptive streaming formats like HLS and DASH and adaptive bitrate algorithms, to provide smooth streaming experiences regardless of device or network conditions. More on that below.
...
Video player lifecycle state machine driven by media events
...
In the following example, the React component state is synced with the DOM video state. Try playing the video using either "Play" button and see that the playback and custom UI states are synced properly.
...
A vanilla <video> with an src attribute is not enough for production
Progressive download cannot adapt quality to bandwidth, support long-form content without huge upfront transfers, or power features like mid-video ad insertion and live streaming. World-class playback requires the Media Source API plus an adaptive streaming protocol (DASH or HLS) on top of a custom player UI.
...
Segmented video files, along with the Media Source API, allow clients to stream video content. This API also allows for the creation of more interactive video experiences, such as the ability to insert ads dynamically, switch between multiple video angles, or synchronize additional content with video playback. Netflix's Bandersnatch is an interactive film with 5 unique endings where users can "choose their own adventure" while watching. As such, the number of combinations is huge, and it is not feasible to create video files for all possible film paths. Using MediaSource helps to dynamically stitch the different parts of the film together depending on the user's choice.
...
Manifest server
Player (client)
[For each segment ahead of playhead]
Renditions, codecs, segment URLs
Create MediaSource, attach via createObjectURL
addSourceBuffer(codec)
Segment bytes
sourceBuffer.appendBuffer(segment)
endOfStream() when manifest exhausted


Manifest fetch and segmented playback via the Media Source API
...
Media Source API | Articles | web.dev
Media Source API | MDN
...
Buffer level ahead of playhead

Viewport size and DPR

Decode capability / codec support
...
#### Higher rendition sustainable?


Step up rendition

Hold current rendition

Fetch next segment at chosen rendition


Adaptive bitrate decision pipeline for the next segment
Start on a low-bitrate rendition and let ABR ramp up
Prioritizing fast startup over initial quality is almost always the right tradeoff: users tolerate a few seconds of lower resolution far better than a blank spinner. Begin playback at a rendition that the current bandwidth estimate can clearly sustain, then allow the ABR algorithm to step up once the buffer is healthy.

Media Capabilities API
The Media Capabilities API allows websites to get more information about the client's video decode performance and make an informed decision about which codec and resolution to deliver to the user.
...
Media Presentation Description (MPD): DASH relies on an XML-based manifest file known as the Media Presentation Description (MPD). The MPD contains metadata about the video content, including information about available quality levels, segment URLs, and other attributes that guide the video player in making adaptive streaming decisions.
...
The dash.js library is a reference client implementation for the playback of DASH via JavaScript and compliant MSE platforms.
...
DASH: In DASH, the manifest file is known as the Media Presentation Description (MPD) file. This file, typically in XML format, contains information about the available quality levels, segment URLs, and other attributes necessary for the video player to request and play the content.
...
Default to DASH on the open web and add HLS for the Apple ecosystem
DASH is codec-agnostic and works wherever the Media Source API is available, which covers most browsers and smart TVs. HLS is still worth supporting because Safari on iOS and older Apple platforms have historically restricted MSE, so HLS is often the only path to native playback there.
...
Used in HLS, an M3U8 file can describe multiple video qualities, allowing the player to switch between different streams based on network conditions or user preferences. Here's an example of an M3U8 playlist with multiple video qualities:
...
How Netflix Pioneered Per-Title Video Encoding Optimization - Streaming Learning Center
...
The differences are subtle, but it is useful to know them. From here on, we will refer to subtitles as a general term to mean on-screen text that accompanies video content.
...
In addition to spoken dialogue, subtitles and transcripts should also identify music and sound effects that communicate important information. This includes emotion and tone:
...
Resources
Implementing Japanese Subtitles on Netflix | by Netflix Technology Blog
...
Preloading media files: Preload media either by using the preload attribute (only for predefined src, not compatible with Media Source API) or use link preload (source).
...
Route resolver
Start hydration
Kick off playback pipeline (in parallel)
Fetch manifest
Prefetch initial segments
Manifest, segments
UI ready, mount <video>
Attach MediaSource, append buffered segments
canplay, show first frame


Parallel UI hydration and playback pipeline for faster startup
Decouple the video pipeline from the UI component tree
Tying manifest fetch, MediaSource setup, and initial segment download to component mount forces the user to wait for hydration before any bytes start flowing. Kick off the playback pipeline as early as possible, ideally the moment the route is known, and attach it to the <video> element once the UI is ready.

Source: Modernizing the Web Playback UI | by Netflix Technology Blog
...
Unbounded SourceBuffer growth will degrade long-lived playback sessions
Appended segments stay in the MediaSource buffer until explicitly evicted, and long binge sessions or background tabs can accumulate hundreds of megabytes of decoded data. Cap buffer ahead of the playhead, evict old ranges behind it, and release the MediaSource fully when the user navigates away to avoid memory pressure and tab crashes on low-RAM devices.
...
Enhanced experience
Prevent layout shifts: Set the width and height attributes on the <video> tag to prevent layout shifts.
...
Poster image: For autoplay videos, YouTube found that using a solid black poster image for autoplay videos was a better experience because it made the transition from solid black to the first frame of the video less jarring.
### Accessibility (a11y)
...
Subtitles
Accessibility for subtitles has been covered in the "Subtitles" section above. To recap:
...
Netflix video capabilities description for screen readers
...
External peripherals: External peripherals can also control the media object state; the web user interface is not the only source of control. The media object's state should be synced with the UI component state so that custom-built controls can accurately reflect playback state.
Source: Accessible multimedia - Learn web development | MDN
...
Language support: The user interface should be translated into multiple languages to support a global audience. Use internationalization libraries or frameworks like i18next or react-intl to manage language translations.
...
Bonus
A couple of production-grade details worth mentioning in an interview if there's time left: seek-bar thumbnail previews and the autoplay billboard on the homepage.

How to serve thumbnails when hovering over the seek bar
...
Netflix on the web is really two distinct products stitched into a single shell: a content-heavy browse surface whose job is to surface the right title as fast as possible, and a stateful watch surface whose job is to keep pixels and audio flowing without stalls. Treat them as separate products with separate constraints, rendering models, and failure modes, and the architecture writes itself. Conflate them, and the watch page pays for SEO work it does not need while the browse page inherits a playback pipeline it never uses.

The browse surface. Server-render the public video title page and the above-the-fold of the discover and recommendations surface so crawlers see real content and the first paint does not wait on JavaScript. Past that, run the app as a single-page application so navigation from a row to a title page to the next row is client-side and feels instant. Hold Recommendations, VideoList, and VideoMetadata in the client store and treat it as a cache that survives back-button navigation and repeat visits inside the session. Bootstrap the React application with inline data on the HTML response (for example, window.netflix.reactContext) so hydration and subsequent store reads never pay an extra round trip for data the server already had. Offscreen thumbnails are lazy-loaded with loading="lazy" and served through responsive images (srcset plus sizes) so a dense grid of box art does not blow the data budget on mid-range mobile.

The watch surface. Default to client-side rendering here: playback is driven end to end by JavaScript through the Media Source API, and there is no SEO value in server-rendering a logged-in player shell. Model the player as a unidirectional reducer with a single source of truth for status, currentTime, bufferedRanges, selectedBitrate, audioTrack, captionTrack, and isFullscreen, and treat <video> as a controlled element so DOM events, media keys, OS controls, and Picture-in-Picture all reconcile back through the same actions. On load, fetch a DASH MPD on the open web or an HLS M3U8 in the Apple ecosystem, create a MediaSource, attach it with video.src = URL.createObjectURL(ms), and append segment bytes into a dedicated SourceBuffer per rendition. Keep audio and video in separate SourceBuffers so an audio-language switch does not restart the video stream and the video stream can be dropped when the tab is backgrounded. The adaptive bitrate loop chooses the next segment from measured throughput, how far the buffer sits ahead of the playhead, the rendered viewport size, and decode support reported by the Media Capabilities API, starting conservative and stepping up only once the buffer is healthy. Serve every segment from a CDN edge close to the viewer, preload the poster for the requested title so the player has something to show while the first segment lands, and render captions as WebVTT delivered through <track> elements with full styling and non-speech cues. Wire every control for keyboard use with visible focus and translated aria-labels, and decouple the playback pipeline from the UI tree so manifest fetch, license acquisition, and the first segment download run in parallel with React hydration rather than behind it.

The browse surface optimizes for recall and first paint: get the right titles in front of the viewer, fast, and keep them there as they navigate. The watch surface optimizes for time-to-first-frame and buffer stability: get the stream playing quickly and never let it stutter.

## References

The references below group video-streaming sources by topic, so it is easier to map each source back to the part of the article it informed.

Video streaming case studies (YouTube, Netflix, Mux)
These references cover how major streaming products approach web playback, encoding, subtitles, and perceived loading performance.

Building a better web - part 1: a faster YouTube on web for how YouTube tunes its web client for faster playback start.
How YouTube improved video performance with the Media Capabilities API for capability-aware quality selection in a real product.
Reverse-engineering YouTube for a concrete look at YouTube's streaming protocol surface.
HTML5 video at Netflix for Netflix's original rationale for moving to HTML5 playback.
Update on HTML5 video for Netflix for how the HTML5 playback stack evolved after the initial rollout.
Modernizing the web playback UI for player UX modernization on the web client.
Implementing Japanese subtitles on Netflix for the subtitle rendering challenges beyond basic WebVTT.
How we made Mux Player's loading feel great for perceived-performance techniques around video loading.
Streaming foundations and protocols
These references cover the underlying concepts and protocols that adaptive video streaming on the web is built on.

How video works for a ground-up explanation of digital video fundamentals.
Digital video introduction for a deeper, code-oriented primer on video encoding and streaming.
Dynamic Adaptive Streaming over HTTP (DASH) for the DASH protocol that underlies many web streaming stacks.
HTTP Live Streaming (HLS) for Apple's HLS streaming protocol and its tooling.
Browser media APIs and standards
These references cover the browser APIs that a web video player integrates with directly.

<video> element | MDN for the core HTML element the player is built around.
<video> and <source> tags | web.dev for practical guidance on authoring video markup.
Media Source Extensions API | MDN for the API that enables segment-driven adaptive playback in the browser.
Media Source Extensions | web.dev for a task-oriented MSE walkthrough.
Setting up adaptive streaming media sources | MDN for configuring DASH and HLS playback in the browser.
DASH adaptive streaming for HTML5 video | MDN for an MDN-hosted DASH integration guide.
Fast playback with audio and video preload for preload strategies that reduce time-to-first-frame.
Lazy loading video for deferring video loads on content-heavy pages.
Player UX and accessibility
These references cover player controls, UI integration, and making video usable for everyone.

UI frameworks and media elements for integrating media elements with modern UI frameworks.
Media accessibility | web.dev for practical guidance on captions, transcripts, and descriptive audio.
Video and audio accessibility for a structured lesson on accessible media.
Open-source players and libraries
These references cover well-known player libraries whose architecture is a useful reference when designing a new player.

Shaka Player for a production-grade DASH and HLS player maintained by Google.
dash.js for the reference DASH client implementation.
Video.js for a long-running HTML5 video player framework.
Media Chrome for composable web components for building custom media UIs.
Related system design articles
The articles below overlap with video streaming on playback, buffering, and media transport.

Music Streaming (e.g. Spotify) for how manifest-driven adaptive playback, buffering, and cross-tab control apply to audio.
Video Conferencing (e.g. Zoom) for how live media transport differs from prerecorded streaming and where player concerns overlap.