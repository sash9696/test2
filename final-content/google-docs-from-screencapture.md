...
tags:
  ...
  - networking
premium: true
summary: "Design a collaborative editor like Google Docs and Notion."
free_preview: false
status: draft
---
...
This article is about collaboration, not text editing itself
This article focuses on the collaboration aspects of collaborative text editing software and not much about text editing itself. For a deep dive into rich text editing, have a look at the Rich Text Editor system design article.
...
Support up to 100 concurrent editors (Google has the same limit)
Background
...
Conflict-prone: During a session, there is a high chance of access conflict as participants work on and modify the same parts of the document, e.g. deleting a sentence while another participant is modifying it.
Distributed: Participants are accessing the web application on their own machines, without any geographical constraints.
Ad hoc: Participants are free to come and go during a session; they may drop off or join at any time.
...
Note that participants should be considered at the browser tab level. A user can open the same document in two separate tabs. Assuming there is no direct communication between browser tabs for the same user, it will be simpler to treat each tab as a separate participant and sync their document states.
...
Approach
It's hard to design an entire complex system by looking at parts in isolation, but it's also not conducive to learning if the final architecture is presented to you without explaining how we arrived at those decisions.

Hence, we will discuss the various aspects of the system, the various approaches that can be taken, and the tradeoffs involved, then make a decision on the overall approach.

It's important to note that some of the decisions we will be making have dependencies on each other, and the decisions only make sense when used together as part of the overall approach. So if, while reading, you're wondering how we know that a certain decision is better, read on first and hopefully the later parts will justify the earlier decisions.
...
DOM with augmented fake cursors: Using regular DOM containing formatted text using HTML elements and styling, augmented with fake cursors. This approach is complex due to the cursor height and positioning calculations needed.
contenteditable attribute: By adding the contenteditable attribute to a DOM element, its content becomes editable and even supports keyboard shortcuts for bold, italics, and underline formatting. However, it behaves differently in different browsers and formatting options are limited.
HTML <canvas> element: This is an advanced approach that uses a <canvas> element and renders everything within the canvas element – text, layout, cursor, etc. This approach basically sidesteps a lot of what the browser provides and requires reimplementing everything within a canvas context.
In practice, most rich text editors on the web are built using the contenteditable attribute backed by custom event handlers, which allows for more content elements.
...
Design an editor-specific model of the document content, usually tree-based
...
Read more about the "front end" of collaborative editors in the Rich Text Editor system design article.
...
Payload contains only changes (the delta): Sending and receiving only the updates made.
...
Sender intentions aren't clear: Just by receiving the updated document, recipients do not know what changes were made unless they compare it with their own document revision or the sender's previous document revision, both of which are troublesome to compute.
...
Small request payload size: Since only the changes are sent, the request payload size is generally small, typically containing only the command (e.g. insertion, deletion, formatting) and associated metadata (e.g. characters and position to insert/delete).
...
A short response time (under 500ms) is required for a real-time WYSIWIS (What-You-See-Is-What-I-See) experience, meaning that when a user makes an edit, all other participants should see that change reflected on their screens within 500ms. If participants see slightly different or outdated versions of the document, the cohesiveness of the session is ruined.

Updates made by participants should be communicated in a lightweight fashion; thus, the request payload for updates should ideally contain only the changes.
...
Peer-to-peer: No central server; all participants act as both server and client
Client-server model
...
![Client-server architecture](google-docs-pdf-assets/figures/client-server-architecture.png)
...
Robustness: When new participants join or one of the existing clients crashes (due to bugs, network flakiness, etc.), the latest document can be fetched from the server.
Disadvantages:
...
![Peer-to-peer architecture](google-docs-pdf-assets/figures/peer-to-peer-architecture.png)


Advantages:

Lower latency: Participants communicate with each other directly without the need for a middleman, resulting in lower latency in general.
...
Complicated consensus: When new participants join or one of the existing clients crashes (due to bugs, network flakiness, etc.), the new member has to figure out how to fetch the latest document revision from among all the peers.
...
Optimize for reliability and performance: Reliability and performance are the most important factors for a seamless collaboration experience and for making sure all participants are updated. In practice, most sessions will not see a large number of participants (> 20) at any given time; there will only be a few participants at most. We should optimize for reliability and performance rather than server costs.
...
Default to a client-server model for collaborative editors
Even if peer-to-peer looks attractive on latency, the server is still needed for durable persistence, a single source of truth, and letting mid-session joiners fetch the latest document from one consistent location. Treat this as the baseline and only consider P2P when losing updates is acceptable, which is almost never true for documents.
...
All these updates have to be merged into the current document. Conflicts might arise if two users are editing the same section of the document (e.g. somebody deletes a sentence while another person is adding a word to the sentence).

Concurrency control is the activity of coordinating interfering actions that operate in parallel to maintain consistency between participants and resolve conflicts that arise between them.

They can be broadly classified into optimistic and pessimistic types:

Pessimistic: Pessimistic methods guarantee that no conflicts occur. Their main objective is inconsistency avoidance. They require communication with other sites or a central coordinator before any data changes are made. This communication can be explicit (e.g. floor control policy) or implicit, where the user's program handles it in the background (e.g. locking). In general, pessimistic methods do not require conflict resolution but have higher network latency and longer response times.
Optimistic: Optimistic methods do not bother with avoiding conflicting updates. They require no prior communication before making local changes and are well-suited for high-latency communication channels since the results of a user's actions can be displayed without waiting for a communications round-trip. The user applies the changes immediately and updates the server, which then notifies the peers. If multiple participants make simultaneous changes, a conflict resolution algorithm creates compensating changes to ensure everyone reaches the same final state. Optimistic methods have zero/near-zero local response times but may require conflict resolution.
...
Do not apply "last write wins" at the document level
Treating the whole document as the unit of concurrency will silently drop one participant's work every time two people save at once. Last-write-wins can still be useful inside narrow CRDT primitives (such as a last-writer-wins register), but it must not be the top-level conflict strategy for a live editor.

Floor control model
"Floor control" is a protocol that determines which user has control (has the floor) and how to take turns when multiple people access a resource, a document in this case.

Token-based: A token circulates among participants, and only the participant holding the token can make changes. Participants can request a token and the predefined policy (e.g. first-come-first-served or free-for-all) determines if the request is granted.
...
In a version-detection model, the document state is replicated on each user's machine and participants can make changes locally, which are then propagated to peers as soon as possible. Under good network connections, users should see updates from peers in under a second.
...
Reject the request: This avoids having to do any conflict resolution but isn't ideal, as the frequency of it happening can be quite high when there are multiple participants simultaneously updating the same part of the document.
...
The version-detection model with compensation enjoys the benefits of zero-latency local updates, all participants being able to update their documents simultaneously in real-time without any locking restrictions, and eventual convergence to the same version of the document if there's a robust conflict detection and resolution approach.

Version detection trumps the transaction-based model and fulfills all the real-time collaboration requirements. The only issue is – how exactly are conflicts resolved? We'll explore that in more detail below.
...
Replicated: The document is replicated on the machines of all participants. This is necessary for the model to make optimistic updates.
...
Apply local edits optimistically against a client-side replica
Users will not tolerate keystroke latency, so the client must commit edits to its own replica first and sync with the server in the background. Locks, pessimistic acquisition, and round-trip-gated edits all couple typing speed to network speed and should be avoided for real-time editors.
...
Alice and Bob network diagram without operational transformations

Alice first: The server receives Alice's request before Bob's, resulting in the server deleting the fourth character and then the second character: "ABCDE" -> "ABCE" -> "ACE". In this scenario, the document on the server correctly deletes Alice's and Bob's intended characters. The server ends up with "ACE".
Bob first: The server receives Bob's request before Alice's, resulting in the server deleting the second character and then the fourth character: "ABCDE" -> "ACDE" -> "ACD". Notice that when the server processes Alice's request, the fourth character of the document is now "E", but Alice wanted to delete "D". The server ends up with "ACD".
...
In fact, Bob will always end up with "ACD" regardless of whose request reaches the server first. Remember that updates are first made locally and then sent to the server to be broadcast to others. The problem is that offsets depend on the state of the document at the time an edit was made. Request payloads include the offsets but not the context they depend on, which results in the divergence.
...
Intention preservation: Ensures that the original intent of an operation is maintained after concurrent operations are merged. This means that the result of merging concurrent operations should align with what users expect their operations to achieve, even in the presence of conflicts. E.g. if Alice makes the entire sentence bold and Bob adds a word to the sentence at the same time, the end result should be that the sentence including Bob's word is bold.
...
Aim to explain principles with an example, not implementation detail
Explaining each conflict resolution approach in detail is beyond the scope of this article (there isn't enough time during interviews anyway). However, you should be able to explain the general principles using an example. We will explain how each approach works and provide examples and links to resources for further reading.
...
Alice and Bob network diagram with operational transformations

Alice before Bob: The server receives Alice's request before Bob's, resulting in the server deleting the fourth character and then the second character: "ABCDE" -> "ABCE" -> "ACE". In this scenario, the document on the server correctly deletes Alice's and Bob's intended characters. However, Bob's computer realizes that Alice's change was made before Bob's deletion, hence it transforms Alice's change from DEL @3 -> DEL @2 since Bob deleted an earlier character. Bob ends up with "ACE".
Bob before Alice: The server receives Bob's request before Alice's, resulting in the server deleting the second character: "ABCDE" -> "ACDE". When the server receives Alice's request, it realizes that Alice's change was made before Bob's deletion, hence it transforms Alice's change from DEL @3 -> DEL @2 to account for Bob's deletion of an earlier character. The server correctly deletes "D" (the third character) and passes this transformed operation to Bob. Both the server and Bob end up with "ACE".
...
Not all changes will conflict, and in those cases they do not require transformation. E.g. FORMAT BOLD @10-20 and FORMAT ITALIC @15-25 do not conflict as text can be both bold and italic at the same time.
...
Intention preservation: OT payloads commonly include the command intention and context (document revision number). The transformation functions consider the context in which an operation was originally applied. The command along with the contextual awareness helps preserve the original intention even when the document has changed due to other concurrent operations.
OT works well for text-based documents but can be less effective for other types of data structures, such as hierarchical or non-linear data. Implementing OT for these types of data requires additional effort and customization.
...
![Visualization of OT with a central server](google-docs-pdf-assets/figures/visualization-of-ot-with-a-central-server.png)

Conflict-free Replicated Data Types (CRDTs)
If you were wondering – why use offset indices for the updates when they are highly coupled to the current document state and go through the hassle of resolving conflicts with complex algorithms like OT? Why not use a different payload or data structure that provides more information regarding the update and makes merging updates easier? That is exactly what Conflict-free Replicated Data Types (CRDTs) aim to do.

CRDTs are advanced data structures designed for distributed systems, enabling multiple users or applications to update shared data concurrently without coordination, which eventually converges to the same state (strong eventual consistency) when all updates have been received and applied. Instead of resolving conflicts using OT, CRDTs are built so that the operations performed on the data are inherently conflict-free or can be automatically resolved in a consistent manner.

CRDTs have the following properties:

Concurrent updates: CRDTs enable independent updates across multiple replicas of data. Each replica can be modified without needing to coordinate with others, making them ideal for environments where network connectivity may be intermittent. Updates can be propagated using the gossip protocol without the need for a central authority.
Monotonically increasing updates: Updates to a CRDT must be monotonic, ensuring that new values are always greater than or distinct from previous values. This allows for a clear progression of state changes.
Commutative and associative operations: The operations in a CRDT must be commutative (order does not matter -> A + B + C === C + B + A) and associative (grouping does not matter -> (A + B) + C === A + (B + C)). This ensures that all replicas, even if they receive operations in different orders or merge states at different times, will end up in the same state.
...
Eventual consistency: Although replicas may have different states at any point in time, CRDTs guarantee that all replicas will eventually converge to the same final state once all updates have been propagated, regardless of the order in which these updates are received. This is often referred to as "strong eventual consistency", which ensures that no inconsistent states are held.
...
Example of CRDT – Grow-only set: An example of a CRDT is a grow-only set. A grow-only set is an unordered set that only allows the addition of unique elements. It is a CRDT because:
...
Representing text in CRDTs: Collaborative text documents can be represented using sequence CRDTs like lists (e.g. Linear Sequences and Replicated Growable Array) and trees. Unsurprisingly, text CRDTs are more complicated to implement than a grow-only set. Cola is a text CRDT written in Rust, but the data structure can be implemented in any language.

Text editing also involves deleting. How can we represent deletion in CRDTs? The trick is to track deletions as well by using two separate grow-only data structures, one to track insertions and another to track deletions (known as tombstone markers). The resulting value is the items in the insertion set minus the items in the deletion set. Complex CRDTs are often combined from smaller CRDTs, which helps tremendously in preserving the CRDT properties.
...
CRDTs entirely bypass the need for causality preservation as updates can be merged in any order and still converge to the same ending state. Whether CRDTs can preserve intention depends on the chosen conflict resolution strategy and implementation.
...
Ecosystem: OT is a mature technology and has been implemented in several well-known collaborative editing systems with many mature libraries and tools available – Google Docs itself uses OT. CRDTs are the new kid on the block, but they have been well-studied over the years and many libraries implementing CRDTs have been created. Figma, a collaborative design editing software, uses CRDTs. There is no clear winner in terms of ecosystem as both approaches have been well-studied (and criticized).

Overall, there's no clearly superior choice. Both CRDTs and OT can be used to implement collaborative editors. CRDTs are general purpose while OT has its roots in document editing. While CRDTs are newer and more trendy, OT is mature and excels in real-time collaborative editing applications where low latency, immediate feedback, and fine-grained control over user intentions are critical.

The rest of the article assumes usage of OT as the conflict resolution approach. The primary reason is that Google Docs itself is implemented using OT, so there are also more resources on implementing collaborative text editors using OT as opposed to CRDTs. It is also easier to explain how OT works as opposed to text CRDTs, which have a very complex structure.

Default to OT for a Google Docs-style editor in interviews
OT is the mature, well-documented choice for linear rich text with a central server and is what Google Docs itself uses. CRDTs shine for decentralized or offline-first systems like Figma, but text CRDTs are harder to explain and carry metadata overhead. Pick OT as the baseline and mention CRDTs as the alternative when the interviewer prompts for tradeoffs.
...
Participants joining: How do users who join an editing session mid-way start updating their replica and receiving updates from others?
A collaboration protocol can be designed to answer these questions. It might be helpful to try out this interactive visualization of OT with a central server before reading on.

Central server
While OT does not require a central server at its core, having a central server simplifies certain things.

A central server architecture makes it simple for clients to stay in sync with the server. The role of the server is to receive updates and serve as the authority of the document state, transforming operations when clients send requests that conflict with the latest document. The server can also reject operations if the payload is invalid, if the operation was on a document revision that is too old, if the operation is too hard to transform, or if there are too many operations to transform.

When the server receives an update request, it will broadcast the update to connected clients. Clients do not need to care about how many other connected clients there are. Each client relies on the server to inform it of the changes to make. From a client's perspective, having N peer clients (where N is the number of peers) take turns every second to make updates in a round-robin fashion is equivalent to a single peer client making updates every second. To a client, having one peer is the same as having ten peers. This allows us to achieve N-way synchronization by running N independent two-party synchronizations between each client-server pair. Clients only have to focus on synchronization with the server, and servers can treat all clients equally; there are no special clients to consider.
...
The document can be stored as an append-only log of operations/changes, similar to the Event Sourcing pattern. Any revision of the document can be reconstructed by replaying the operations from the start up to that point. Hence, the latest revision of the document can be obtained by replaying all the operations since the start. This method of storing data also allows clients to view the document's version history.
...
Storing as a log is necessary since some clients can be extremely outdated and need to catch up with the current document. Imagine the scenario where a user disconnects from a collaboration session and leaves the tab open. When they reconnect the next day and others have made updates since then, upon reconnecting, the client sends its current outdated document revision number, which allows the server to determine the granular changes since that document revision and respond with a list of operations to be performed on the client, which might also need transformation.

Clients that join the editing session of a non-empty document do not need to fetch the entire log from the start; the server responds with the current state of the document. Subsequent document revisions can be computed by applying new operations (both local and from peers) to the initial session document state.
...
Keystroke: Send a request per keystroke. Not great because it can be taxing on the server and many potentially redundant operation logs are created.
...
There are ways to resolve out-of-order requests, but they aren't trivial. The client can include a monotonically increasing integer for every request in the session, but the server has to keep track of the last request sent so far, and if it receives an out-of-order request, defer the future request and then wait for the missing request (buffering future operations).

An elegant approach taken by Google Docs is to ensure that each user only has a maximum of one update request in-flight (aka pending) by using a local updates buffer (a queue). The local updates buffer is cleared and operations in the buffer are sent to the server under one of the following scenarios:
...
User operations are reflected locally on the client computer and added to the local updates buffer
After a short duration, a request is made that includes the operations in the local updates buffer
The local updates buffer is cleared upon sending of the request, and any new local updates made while there's a pending request are added to the local updates buffer
If the local updates buffer is not empty, it only sends the next request after it receives a response for the current request from the server, even if the idle duration has passed
This repeats until the local updates buffer is empty (when the user has stopped typing) and there are no more operations to send to the server
...
Fast vs slow connection for edit requests

Fast connection: Clients with a faster network connection speed will send requests and receive responses faster, resulting in more frequent requests and each request containing a smaller payload. The local updates buffer will be cleared more often.
...
Append operation
Flush after idle or prior ACK
UPDATE { revision, ops }
More keystrokes while pending
Append ops (held back)
ACK { revision + 1 }
Clear acknowledged ops
Flush next batch
UPDATE { revision + 1, ops }


Local-first edits with one in-flight update request
Keep at most one update request in-flight per client
Parallel in-flight updates let the server observe operations out of order, which breaks OT's causality assumptions and can produce impossible transforms. Buffer further local edits while a request is pending and flush the buffer only after the server acknowledges; this is a cleaner contract than reordering on the server.

Operation granularity
A suitable operation granularity is one which doesn't result in too many operations yet also allows differentiation between intentions. Consider the following scenarios:
...
If a user types some characters, realizes there's a typo, deletes the erroneous characters, and then types again, these actions constitute "continuous typing" and could be coalesced into a single insertion operation. In this case, when the operations are sent to the server, the server does not see any deletions at all.
...
It wasn't explicitly mentioned, but each update payload can contain multiple operations. On slower network connections where request round-trips take longer, there is a larger window for users to make local updates, and sometimes that can include different kinds of operations, not just insertions.
...
Each client tracks the following information:

Latest document revision: The identifier of the most recent revision sent from the server to the client. Google Docs uses a monotonically increasing integer as the document revision id/number. This value is included in the request payload and response of each update request.
Local updates buffer: Changes (operations) that have been made locally and not yet sent to the server. This is the updates buffer explained earlier.
Sent updates buffer: Changes (operations) that have been made locally and sent to the server, but not yet acknowledged by the server. Tracking sent operations is necessary because requests can fail and the client should retry the request if a request fails.
Updates log: Committed changes (operations) for the document since the initialized document revision. These changes could be the user's updates that have been acknowledged, or updates by peers that have been pushed to the client by the server. The operations can be already transformed on the server, or transformed on the client depending on the order of updates.
Initial document state: The state of the document when the client first joined the editing session.
Document state: The current state of the document displayed on the client. This can be computed from the initial document state, the received updates, the updates buffer, and the pending updates.
...
Combine the initial document state with the updates log to compute the latest document state and display it.
The server contains the following information:

Pending updates queue: A list of all changes (operations) received from clients that have not been processed.
Revision log: A list of processed changes that gives the complete history of the document.
Document state: The current state of the document as of the last processed change. This value can be derived by replaying all the changes since the start but is computed and cached so that it can be instantly sent to new clients that join the session. It is recomputed whenever a new change is processed.
The server's responsibilities are to:

Send clients the latest state of the document when they join the editing session.
Transform updates from clients if they were originally made on outdated revisions.
Broadcast updates to the other clients.
Source: What’s different about the new Google Docs: Making collaboration fast
...
Alice, Bob, and the server start with an empty document
Alice types "Hello" and the insertion operation is added to her "Local updates". Alice sees "Hello" instantly within her document replica
Alice's insertion of "Hello" is sent to the server and moved from "Local updates" to "Sent updates"
The server receives the request and adds Alice's insertion operation to its "Pending updates" queue
At the same time, Alice types the characters " world". This insertion is added to "Local updates" but will not be sent to the server until "Sent updates" is empty
The server processes Alice's first insertion and updates its document state. It then sends an acknowledgement to Alice
Alice removes that operation from "Sent updates" and updates her latest revision number to 1
The server broadcasts Alice's insertion to Bob, and Bob applies the operation to his document replica and updates his latest revision number to 1
Collaboration part 2

Alice's second insertion of " world" can now be processed. The operation is sent to the server and moved from "Local updates" to "Sent updates"
The server receives Alice's second request and adds her operation to its "Pending updates" queue
At the same time, Bob inserts a "!" character at the end of "Hello"
Bob's insertion of "!" is sent to the server and added to the server's "Pending updates" queue. Bob moves the operation from "Local updates" to "Sent updates"
The server processes Alice's second insertion first and sends an acknowledgement to Alice
Alice removes that operation from "Sent updates" and updates her latest revision number to 2
The server broadcasts Alice's insertion to Bob. However, Bob has uncommitted updates, so he needs to transform them against Alice's updates. Bob's insertion of "!" is transformed to the 11th position to make room for Alice's " world". Bob updates his latest revision number to 2
The server processes Bob's insertion next. It sees that Bob's operation was made against revision 1, which does not account for Alice's second insertion. Hence, the server transforms Bob's insertion of "!" to the 11th position to make room for Alice's " world". This shift is identical to the transformation Bob's client made when it first received Alice's insertion of " world"
The server processes Bob's transformed insertion and sends an acknowledgement to Bob. Bob removes that operation from "Sent updates" and updates his latest revision number to 3
The server broadcasts Bob's insertion to Alice. It has already been transformed, so Alice can simply apply the operation and update her latest revision number to 3
Alice, Bob, and the server all end up with the same version of the document.

Client logs mirror the server's revision log
The update logs of Alice and Bob aren't shown in the diagrams, but they mirror the server's revision log.
...
Ordered: Servers assume that updates broadcast to clients will be received in the order they are sent, as transformations are made with the assumption that the operations are made in order. Hence, the transport mechanism needs to guarantee that messages are sent in order during broadcast.
...
Ordering: None of the mentioned transport mechanisms guarantee in-order delivery of messages. Client-to-server communication is guaranteed thanks to client-level message ordering by enforcing only one in-flight request at a time. Server-to-client communication ordering is required by the communication protocol but not built into it. In SSE, if the connection drops, it is automatically retried, but that's not the case for other methods.

Overall, WebSockets are the best choice of transport mechanism for collaborative editing given their low-latency and bidirectional communication properties. WebSockets are widely used by many production-grade apps for real-time updates. Although WebSockets do not guarantee message ordering, that gap can be filled with custom logic in the application layer via libraries like Socket.io.

Default to WebSockets and add ordering plus retries in the app layer
WebSockets give you the bidirectional, low-latency channel the protocol needs, but do not assume the transport itself provides in-order delivery, retries, or acknowledgements. Layer those guarantees on top with request IDs, revision numbers, and an ACK protocol; libraries like Socket.io are a reasonable shortcut for the boilerplate.
...
Long polling is not viable given that there are more modern technologies that overcome its shortcomings.
...
Conflict resolution via OT: OT is chosen due to its maturity, and OT has its roots in document editing. Each operation provides enough information for merging updates. Google Docs has also stuck with OT technology over the years and is still using OT to this day.
...
Update scheduling: Each client maintains a "local updates" buffer and a "sent updates" buffer, ensuring that there will only be one network request in-flight at any time.
Operation granularity: Operation granularity is decided at the intention level and continuous typing actions are coalesced.
...
We highly recommend checking out this interactive visualization of OT with a central server; it really helps with understanding the collaboration protocol.
...
Central server that all clients communicate with. Clients do not communicate with each other directly.
...
Sync engine: Module responsible for syncing local updates to the server, receiving updates from the server (transforming appropriate operations), and updating the editor core. Most of what was discussed above lives in this module.
...
The editor core and UI are entirely decoupled from the sync engine and server back end. The editor core provides APIs for the sync engine to modify the document model depending on the received operations.


UI / rich text view

Editor core
tree model + DOM binding

Sync engine
local + sent buffers, updates log
...
Collaborative editor architecture layers
## Data model
...
However, within the sync engine, the document is stored as the initial document state plus multiple lists (sequences) of operations (local, sent, received). The latest document state can be constructed by applying these operations on top of the initial document state. Each committed operation increases the revision number.
...
| --- | --- | --- | --- | --- |
| 3 | Bob | INS "!" @11 | 2024-08-03 10:15 | "Hello world!" |
When a new user joins the session or if the current user refreshes the page, the document they initialize with is the latest version, constructed from all the committed operations on the server; all client-side operation lists will be empty.
...
Bob then joins the session at revision 2 and he is directly initialized with the current document state of "Hello world", without any granular operation history.

Operational transformations on the document level
So far, the operations (and transforms) we have mentioned in this article are executed at the sentence level in a plaintext context. However, documents are rich text; how can transformations be performed on documents?
...
A full rich text document is represented as a tree, and there are two categories of nodes – element nodes and text nodes. Element nodes can contain child nodes, which are other element nodes or text nodes, while text nodes are leaves and can only contain textual content (plaintext) and have formatting flags. Headings and paragraphs are subclasses of element nodes because they can contain child text nodes. At a high level, a document contains a root node with a list of heading elements, paragraph elements, etc. as its direct children.
...
Document, revision, and client-side sync engine entities
...
Update/save API
This API allows sending local updates to the server. Since this is done using WebSockets, a type field is necessary to differentiate between the requests. Note that multiple operations are allowed in a single update request.
...
When clients disconnect and finally reconnect, the server should send them any revisions they missed while disconnected.
...
With the core architecture in place, this section dives into collaboration concerns that build on the operation log, revision model, and real-time sync flow:

History and versioning: This section explains how the operation log supports document history, named versions, and time travel.
Undo/redo: This section covers why undo and redo are harder in collaborative editors and how user-scoped intent affects the design.
Reliability: This section discusses ordered delivery, reconnection, acknowledgements, and duplicate handling over flaky networks.
Offline editing: This section explains how local edits can queue while disconnected and reconcile once connectivity returns.
Document formats: This section covers import and export tradeoffs for formats such as DOCX and ODT.
History and versioning
Google Docs allows users to view the document history as a list of versions. By storing the document as a log of operations/updates, we can "time travel" and go back to the document state at any point in time. Each document revision is identified by a monotonically increasing positive integer, which is constructed from the operations up to that point.
...
Although every granular change is stored in the database, it is more meaningful to display a version log that groups multiple revisions together. Changes made together within a short duration are part of the same version:
...
Should a user's undo/redo history be at the user level or the document level (all participants share the same undo/redo)? It makes more sense to only undo your own actions, as users are unlikely to be aware of what others are doing and want to undo them.
...
Hence, update operations need an isUndo flag to differentiate between new operations and undo operations, and filter them out when identifying their last non-undo operation; otherwise, users will be stuck undoing/redoing (undoing the undo) the most recent operation.

Undo must scope to the current user, not the whole document
Users expect Ctrl+Z to undo only their own actions, not a peer's. The client filters its updates log for its own non-undo operations and appends negations as new operations; document-wide undo would be disorienting and would also require peers to reverse transforms that have already propagated.
...
Reliability
Reliability is about ensuring that messages sent from the server are guaranteed to be received by the client in the order they were sent. Clients can disconnect at any time, resulting in the possibility of the server sending a message that is not received by the client. Clients can also be on unstable network connections, and some messages get lost in transit.

Dropped messages are not an issue if the messages contain the entire document state, but as explained above, that isn't efficient. In our approach, messages contain crucial granular updates, and missing any of them will result in document replicas going out of sync. Clients have to receive all peer operations in order to converge to a consistent document state.
...
If a client reconnects to the session with revision 5 in the payload and the server is currently on revision 8, the server knows that the client is missing revisions 6, 7, and 8, and thus can send the updates for revisions 6, 7, and 8.

Let the client carry its revision number, not the server
Storing each client's last-seen revision on the server forces it to track per-connection state and scales poorly. When the client supplies its revision on every request and reconnect, the server can remain stateless with respect to clients and still compute exactly which operations to replay.

Other reliability requirements include in-order delivery, retries, and acknowledgements. WebSockets do not include these features, so custom logic has to be added in the application layer via libraries like Socket.io.

The collaboration protocol outlined above can handle flaky connections well. As long as the client retains the document revision number and sends it to the server as part of the request payload, the server will be able to determine what updates are missing from the client. Each update operation also has a unique ID tagged to it, which facilitates de-duplication in the case of duplicate requests.

socket open

initial doc + revision received

socket drop / network loss

missed peer ops detected

network restored

socket reopened, send last revision

server replays missing revisions

reconnect failed, retry with backoff
...
Client connection lifecycle across flaky networks
Offline editing
...
Send local updates to server: The "local updates" are sent to the server and moved to the "sent updates" buffer.
Fetch updates from server: The client could have missed some updates while it was offline; the server should push the missing updates since the client's last synced revision.
Client (sync engine)
Client (sync engine)
### Network drops, client enters offline

### Network restored, reopen socket

Continue editing
Append ops to local buffer, apply to replica
HELLO { lastRevision: 5 }
SYNC { revisions 6..8 }
Transform local ops against missed revisions
UPDATE { baseRevision: 8, ops }
Apply + transform against concurrent peers
ACK { revision: 9 }
Broadcast revision 9 to peers


Offline reconnect and catch-up sync
Document formats
There are numerous file formats that are supported by word processors like Google Docs and Microsoft Word. Of these, the .docx (Microsoft Word) and .odt (OpenDocument Text) file formats are the most popular.

The specifications for the .docx and .odt file formats are openly available:

.docx: Word Extensions to the Office Open XML (.docx) File Format
.odt: OpenDocument technical specification
...
A collaborative editor is really four hard problems stacked on top of each other: getting concurrent edits from many clients to converge on one document, recovering cleanly when a client reconnects after being offline, giving each user an undo stack that only touches their own work, and moving all of this over a transport that will drop messages and reorder them. Most other features become easier to place once those four are handled well.

Make concurrent edits converge through a central server and revision log. Clients do not synchronize peer-to-peer. A central server owns the document, and each client keeps a local replica that it edits optimistically so typing is never gated on the network. Conflicting edits are resolved with Operational Transformations against an append-only revision log, with every update carrying the base revision it was authored against.

Treat offline reconnection as a longer local update buffer. A disconnected client keeps appending to that buffer, and on reconnect the SYNC message sends the last-seen revision so the server can replay missed revisions before the client flushes its queued operations. The revision log is what makes this replay cheap and deterministic.

Keep undo personal by appending inverse operations. Undo does not rewind the document to an earlier state, which would clobber peers. Instead, the client appends the negation of the user's own most recent non-undo operation as a new operation, tagged with isUndo so redo and further undo can walk the stack without folding the undo operations back into themselves. Each user's undo history stays local and personal even as the shared document moves forward.

Add reliability at the application layer, not by trusting WebSockets. Each client keeps at most one update in-flight, tags it with a requestId for de-duplication, and waits for an ACK before sending the next. Peers see the change as a PEER_UPDATE broadcast. Reordering, duplicates, and brief disconnects are absorbed by the revision numbers and request IDs rather than by WebSockets themselves.

Keep document file formats at the import/export boundary. The .docx and .odt formats never touch the live editing path, so the internal tree model is free to evolve independently.

Start with the collaboration protocol: client-server, optimistic local edits, OT against a revision-numbered append-only log, and one in-flight update per client. Undo, offline, versioning, and format interop extend from that core flow as the interviewer presses for depth.

## References

The references below group Google Docs product and engineering posts, OT and CRDT foundations, case studies from adjacent collaborative tools, transport and format specs, and related system design write-ups on this site.

Google Docs product and engineering
These posts describe how Google Docs frames the collaboration problem and the constraints that shape the front-end architecture.

What's different about the new Google Docs: Working together, even apart for the high-level collaboration model.
What's different about the new Google Docs: Conflict resolution for OT-based conflict resolution.
What's different about the new Google Docs: Making collaboration fast for latency and performance tradeoffs.
Improved accessibility in Google Docs and Sites for accessibility considerations in collaborative editors.
Google Workspace: Up to 100 concurrent editors for the concrete scale constraint cited in requirements exploration.
OT, CRDTs, and concurrency foundations
These sources cover the algorithms behind automatic conflict resolution in real-time collaborative editors.

Concurrency Control in Groupware Systems for the foundational OT paper.
High-Latency, Low-Bandwidth Windowing in the Jupiter Collaboration System for the Jupiter protocol, the direct ancestor of the Google Docs approach.
Operational Transformations as an algorithm for automatic conflict resolution for an accessible walkthrough of OT.
Interactive visualization of OT with a central server for hands-on intuition for how OT resolves concurrent edits.
Operation-based CRDTs: Arrays (Linear Sequences and RGA) for text-sequence CRDT fundamentals.
Cola: a text CRDT in Rust for practical text CRDT implementation notes.
Case studies from other collaborative tools
These write-ups describe how adjacent products solve overlapping collaboration problems and are useful for comparing design choices.

How Figma's multiplayer technology works for multiplayer architecture in a non-document-text setting.
Realtime editing of ordered sequences for handling ordered collections under concurrent edits.
Understanding sync engines: how Figma, Linear, and Google Docs work for a comparative overview of sync engines across products.
The data model behind Notion for document data-model tradeoffs relevant to block-structured editors.
A pragmatic approach to live collaboration for shipping collaboration without a full OT or CRDT engine.
Transport, persistence, and document formats
These specs and libraries back the transport, storage, and export decisions discussed in the article.

Event Sourcing pattern for background on the operation-log storage model.
Socket.io for a WebSocket wrapper that adds ordering, retries, and reconnection on top of raw WebSockets.
Word Extensions to the Office Open XML (.docx) File Format for the .docx format spec.
OpenDocument technical specification for the .odt format spec.
Related system design articles
These articles on this site cover adjacent problems and share parts of the collaboration backbone.

Collaborative Spreadsheet (Google Sheets) for an adjacent collaboration problem involving formulas, ranges, and two-axis virtualization.
Design a Drawing Tool (Figma / Canva) for collaboration in a canvas-based editor rather than text.
Rich Text Editor for the single-user editor-surface tradeoffs that underlie the collaborative case.